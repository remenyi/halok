...
interface <interfész>
switchport mode access
mls qos trust cos # Az előző parancs helyett, ha VoIP-et akarunk
switchport mode access vlan <vlan szám>
...
show vlan [name <név>]
show vlan summary
show interfaces vlan <vlan szám>
#+END_SRC
...
switchport trunk native vlan <natív vlan szám>
swichport trunk allowed vlan <engedélyezett vlan szám...>
end
#+END_SRC

** Trönkök törlése interfészről
Global config mode-ban kell lenni
#+BEGIN_SRC sh
interface <interfész>
...
Global config mode-ban kell lenni
#+BEGIN_SRC sh
interface <interfész>.<vlan szám>
# Alinterfész mode-ba kerülünk
encapsulation dot1q <vlan szám>
